...
import { Send, Hash, Users, Server, LogOut, ArrowLeft } from "lucide-react";
...
export default function ChatPage() {
  ...
  const { isConnected, messages, userId, nodeId, onlineCount, error, sendMessage } = useWebSocket(username);
  ...
  return (
    <div className="flex h-screen bg-[var(--color-bg-primary)] overflow-hidden text-[var(--color-text-primary)]">
      ...
      <div className="w-[72px] bg-[var(--color-bg-tertiary)] flex flex-col items-center py-4 gap-4 flex-shrink-0 hide-scrollbar overflow-y-auto hidden md:flex border-r border-white/5">
        ...
        <div className="w-12 h-12 bg-[var(--color-bg-secondary)] rounded-[24px] hover:rounded-[16px] transition-all flex items-center justify-center cursor-pointer group hover:bg-[var(--color-brand-start)] text-white/50 hover:text-white">
      ...
      <div className="w-60 bg-[var(--color-bg-secondary)] flex flex-col flex-shrink-0 hidden sm:flex border-r border-white/5">
        <div className="h-12 border-b border-white/5 flex items-center px-4 font-bold shadow-sm">
          PDC Distributed Network
        </div>
        <div className="flex-1 overflow-y-auto py-4 px-2 space-y-1">
          <div className="px-2 text-xs font-semibold text-[var(--color-text-muted)] mb-2 uppercase tracking-wider">
            Text Channels
          </div>
          <button className="w-full flex items-center gap-2 px-2 py-1.5 bg-white/5 text-[var(--color-text-primary)] rounded-md cursor-default">
            <Hash className="w-4 h-4 text-[var(--color-text-muted)]" />
            global-chat
          </button>
          <button className="w-full flex items-center gap-2 px-2 py-1.5 text-[var(--color-text-muted)] hover:bg-white/5 hover:text-[var(--color-text-primary)] rounded-md transition-colors">
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-[var(--color-bg-primary)]">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          ...
          <div className="mb-8">
            ...
            <h1 className="text-3xl font-bold text-white mb-2">Welcome to #global-chat!</h1>
            <p className="text-[var(--color-text-muted)]">This is the start of the distributed messaging network.</p>
          </div>
      ...
      <div className="w-60 bg-[var(--color-bg-secondary)] flex-shrink-0 hidden lg:flex flex-col border-l border-white/5">
        <div className="p-4">
          ...
          {/* Mock visual list mapping just one self user for MVP, since backend doesn't broadcast all usernames yet */}
          <div className="flex items-center gap-3 px-2 py-2 hover:bg-white/5 rounded-md cursor-pointer transition-colors group">
            <div className="relative">
              <div className="w-8 h-8 rounded-full bg-gradient-to-br from-[var(--color-brand-start)] to-[var(--color-brand-end)] flex items-center justify-center font-bold text-white uppercase">
                {username.charAt(0)}
              </div>
            ...
            <span className="text-[15px] group-hover:text-white text-[var(--color-text-primary)] truncate">
              {username} <span className="text-xs text-[var(--color-text-muted)] opacity-0 group-hover:opacity-100">(You)</span>
            </span>
          </div>
          
          {/* Mock random users for visual pop if onlineCount > 1 */}
          {onlineCount > 1 && Array.from({length: Math.min(onlineCount - 1, 5)}).map((_, i) => (
             <div key={i} className="flex items-center gap-3 px-2 py-2 hover:bg-white/5 rounded-md cursor-pointer transition-colors opacity-70 group mt-1">
             <div className="relative">
               <div className="w-8 h-8 rounded-full bg-[#2f3136] flex items-center justify-center font-bold text-[var(--color-text-muted)] uppercase ring-1 ring-white/10">
                 U
               </div>
               <div className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 rounded-full border-2 border-[var(--color-bg-secondary)]" />
             </div>
